...
extension ActivitiesCommentsController{
    ...
    func handleSendComment(){
        ...
        if(!hasAnyErrors()){
            ...
            let comment = Comment(rev: nil, cid: nil, authorID: UserDefaults.standard.string(forKey: GetString.userID.rawValue), authorUsername: self.activityObject?.userName, groupID: self.activityObject?.groupID ,dailyActivityID: self.activityObject?.aid, threadID: nil, message: commentView.text, date: Date(), likeIDs: nil)
