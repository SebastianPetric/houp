...
class TimerObject{
    ...
    @objc func fireTimerHandleCreateActivity(){
        let state = UIApplication.shared.applicationState
        if state == .active {
            if(self.tabContr?.selectedIndex != 2){
                self.tabContr?.selectedIndex = 2
    ...
    func setUpTimerCreateActivity(){
        self.timer = Timer(fireAt: Date(), interval: 20, target: self, selector: #selector(fireTimerHandleCreateActivity), userInfo: nil, repeats: true)
        RunLoop.main.add(self.timer, forMode: RunLoopMode.commonModes)
    }

    
    func setUpTimer(date: Date){
        self.timer = Timer(fireAt: Date().checkIfActivityAlreadyOver(date: date), interval: 10, target: self, selector: #selector(fireTimer), userInfo: nil, repeats: true)
        RunLoop.main.add(self.timer, forMode: RunLoopMode.commonModes)
    }
    
    //wenn er gerade keine zeit hat die formulare auszufüllen
    func setUpTimerToDelayForms(){
        self.timerToDelay = Timer(fireAt: Date(), interval: 10, target: self, selector: #selector(tryAgain), userInfo: nil, repeats: true)
