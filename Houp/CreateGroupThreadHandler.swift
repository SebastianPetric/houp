...
extension CreateGroupThreadController{
    ...
    func handleCreate(){
        if(hasAnyErrors()){
            ...
            groupID = "0"
            let thread = Thread(rev: nil, tid: nil, authorID: uID, authorUsername: nil, groupID: groupID, title: self.titleThread.text!, message: self.messageThread.text!, date: Date(), commentIDs: nil)
            threadsList.append(thread)
            }else if(self.sendToAllGroupsSwitch?.isOn)!{
                if let groupsList = self.privateGroupsList{
                    for group in groupsList{
                        let thread = Thread(rev: nil, tid: nil, authorID: uID, authorUsername: nil, groupID: group.pgid, title: self.titleThread.text!, message: self.messageThread.text!, date: Date(), commentIDs: nil)
                        threadsList.append(thread)
                    }
                }
            }else{
